...
public class c8_Sinks extends SinksBase {
    ...
    public void single_subscriber() {
        ...
        submitOperation(() -> {
            ...
            measures_readings.forEach(measure -> sink.emitNext(measure, Sinks.EmitFailureHandler.FAIL_FAST));
    ...
    public void emit_failure() {
        ...
        for (int i = 1; i <= 50; i++) {
            int finalI = i;
            new Thread(() -> sink.emitNext(finalI, ((signalType, emitResult) -> emitResult == Sinks.EmitResult.FAIL_NON_SERIALIZED))).start();
        }
